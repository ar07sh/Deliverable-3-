...
public class Uno {
	...
	private UnoCards current; // the current card or previously played card or starting card
	private GroupOfCards deck; // the deck of the game
	private ArrayList<UnoCards> cardpile; //when players throw card, it piles up here. Also, creates a new deck if the current deck is empty
	...
	private UnoPlayer p1,p2; //player 1 and 2
	...
	public Uno() {
		...
		deck.shuffle();
		penalty = 0;
		current = (UnoCards) getStartingCard();
		cardpile = new ArrayList<UnoCards>();
		cardpile.add(current);
		choice = new Scanner(System.in);
		p1 = new UnoPlayer("Player 1");
		p2 = new UnoPlayer("Player 2");
		distributecards();
...
private void distributecards() {
	...
	public void playGame(UnoPlayer p) {
		...
		*/
		...
		if(current.isSpecial()) {
			...
			if(!canOverride(p)) {
				...
				penalty = 0;
				current = (UnoCards) deck.getTopCard();
...
//		}   
		...
		UnoCards play = p.throwCard(pick);
...
//		reviveDeck();
	...
	private boolean hasSpecial(UnoPlayer p) {
		// TODO Auto-generated method stub
		
		for(UnoCards c:p.PlayerCards()) {
	...
	private boolean isValidChoice(UnoPlayer p, int choice) {
	...
	private boolean hasColor(UnoPlayer p) {
		/*
	...
	private boolean hasValue(UnoPlayer p) {  
		/*
	...
	private boolean canOverride(UnoPlayer p) {
	...
	private Card getStartingCard() {
		...
		UnoCards temp = (UnoCards) deck.peek();
		while(temp.isSpecial()) {
			deck.shuffle();
			temp = (UnoCards) deck.peek();
		}
		
		temp = (UnoCards) deck.getTopCard();
		return temp;
	}
...
public boolean gameOver(UnoPlayer p1, UnoPlayer p2) {
